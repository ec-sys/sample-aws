package demo.aws.sample.kafka.utils;
...
public class KafkaUtil implements AutoCloseable {
    ...
    public void changeConfig(final Properties allProps, List<NewTopic> topics)
            throws InterruptedException, ExecutionException, TimeoutException {
        try (final AdminClient client = AdminClient.create(allProps)) {
            ConfigResource configResource = new ConfigResource(ConfigResource.Type.TOPIC, topics.get(0).name());
